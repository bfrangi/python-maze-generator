...
class MazeGenerator:
	...
	def maze_to_string(self,
					   ...
					   highlight_char='██', show_solution=False):
			...
			# 1.2 Handle even index rows (rows with no cells)
				...
				for j in range(width):
					...
					# 1.2.1.2 Handle odd columns - may be open or closed
						...
						x_idx = j//2
						y_idx = i//2-1
						box = self.MAZE[self.maze_map_index_from_coordinates(
							x_idx, y_idx)]
						if box[1]:
							ch = wall
						else:
							...
							if (
								...
								ch = highlight_color + highlight_char + MazeColors.ENDC
						maze_str += ch
				maze_str += '\n'
	...
	def check_advance_direction(self, curr_x, curr_y, direction):
		if direction == 0:  # North
			...
			if (new_x, new_y) in self.CELL_STACK + self.DEAD_ENDS:
				return False, None, None
			# Then check walls
			north_wall = self.MAZE[self.maze_map_index_from_coordinates(
				new_x, new_y)][1]
			if not north_wall:
				return False, None, None
			...
			if (new_x, new_y) in self.CELL_STACK + self.DEAD_ENDS:
				return False, None, None
			# Then check walls
			east_wall = self.MAZE[self.maze_map_index_from_coordinates(
				curr_x, curr_y)][0]
			if not east_wall:
				return False, None, None
			return True, new_x, new_y
			...
			if (new_x, new_y) in self.CELL_STACK + self.DEAD_ENDS:
				return False, None, None
			# Then check walls
			south_wall = self.MAZE[self.maze_map_index_from_coordinates(
				curr_x, curr_y)][1]
			if not south_wall:
				return False, None, None
			return True, new_x, new_y
			...
			if (new_x, new_y) in self.CELL_STACK + self.DEAD_ENDS:
				return False, None, None
			# Then check walls
			west_wall = self.MAZE[self.maze_map_index_from_coordinates(
				new_x, new_y)][0]
			if not west_wall:
				return False, None, None
			return True, new_x, new_y
	...
	def print_maze_step(self, pause=False,
						wall='██', space='  ', color_walls='',
						color_spaces='', highlight_color=MazeColors.FAIL, highlight_char='██'):
		str_maze = self.maze_to_string(wall=wall, space=space,
									   color_spaces=color_spaces,
									   color_walls=color_walls,
									   highlight_color=highlight_color,
									   highlight_char=highlight_char,
									   highlighted_cells=self.CELL_STACK)
		print(str_maze)
		if pause:
	...
	def advance(self, curr_x, curr_y, print_steps=False, pause=False, log=False,
				wall='██', space='  ', color_walls='', color_spaces='',
				highlight_color=MazeColors.FAIL, highlight_char='██'):
		directions = self.random_directons()
		if print_steps:
			self.print_maze_step(pause=pause, wall=wall, space=space,
								 color_walls=color_walls, color_spaces=color_spaces,
								 highlight_color=highlight_color, highlight_char=highlight_char)
		for direction in directions:
			can_advance, new_x, new_y = self.check_advance_direction(
			...
			if can_advance:
				...
				return self.advance(
					new_x, new_y, print_steps=print_steps, pause=pause, log=log,
					wall=wall, space=space, color_walls=color_walls, color_spaces=color_spaces,
					highlight_color=highlight_color, highlight_char=highlight_char)
		if print_steps:
			print(f'Cannot advance... backtracking')
		return self.backtrack(curr_x, curr_y, print_steps=print_steps, pause=pause, log=log,
							  wall=wall, space=space, color_walls=color_walls, color_spaces=color_spaces,
							  highlight_color=highlight_color, highlight_char=highlight_char)

	def backtrack(self, curr_x, curr_y, print_steps=False, pause=False, log=False,
				  wall='██', space='  ', color_walls='', color_spaces='',
				  highlight_color=MazeColors.FAIL, highlight_char='██'):
		...
		if log:
			with open(log, 'a') as f:
				f.write(msg + '\n')
		return self.advance(previous_cell[0], previous_cell[1], print_steps=print_steps, pause=pause, log=log,
							wall=wall, space=space, color_walls=color_walls, color_spaces=color_spaces,
							highlight_color=highlight_color, highlight_char=highlight_char)
	...
	def generate_maze(self,
					  ...
					  wall='██', space='  ',
					  color_walls='', color_spaces='',
					  highlight_color=MazeColors.FAIL,
					  highlight_char='██'):
		if log:
		...
		maze = self.advance(
			...
			print_steps=print_steps, pause=pause, log=log,
			wall=wall, space=space, color_walls=color_walls, color_spaces=color_spaces,
			highlight_color=highlight_color, highlight_char=highlight_char
		)
		return maze
